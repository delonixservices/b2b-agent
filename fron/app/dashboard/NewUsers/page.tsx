...
import { Plus, Users, UserX, UserCheck, Phone, Lock, User, Trash2, Edit, Eye, X, RefreshCw, List, Percent, UserPlus, Repeat, CreditCard, LogOut } from 'lucide-react'
  ...
  const fetchEmployees = async (showRefreshSpinner = false) => {
    try {
      ...
      if (err.response?.status === 403) {
        setError('Access denied. Only companies can view employees.')
      } else {
  ...
  const handleAddEmployee = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (err.response?.status === 400) {
        ...
        setError('Access denied. Only companies can add employees.')
      } else {
  ...
  const handleDeactivateEmployee = async (employeeId: string) => {
    ...
    try {
      ...
      if (err.response?.status === 404) {
        ...
        setError('Access denied. Only companies can deactivate employees.')
      } else {
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col">
      <div className="flex flex-1">
        {/* Sidebar */}
        <aside className="hidden md:flex flex-col w-64 bg-white shadow-lg py-8 px-4 fixed h-full z-40">
          <div className="mb-10 text-center">
            <span className="text-2xl font-bold text-blue-600">B2B Dashboard</span>
          </div>
          <nav className="flex flex-col gap-4">
            <button onClick={() => router.push('/dashboard/details')} className="flex items-center gap-3 px-4 py-2 rounded-lg hover:bg-blue-50 text-gray-700 font-medium">
              <List className="w-5 h-5" /> Details
            </button>
            <button onClick={() => router.push('/dashboard/Markup')} className="flex items-center gap-3 px-4 py-2 rounded-lg hover:bg-blue-50 text-gray-700 font-medium">
              <Percent className="w-5 h-5" /> Markup
            </button>
            <button onClick={() => router.push('/dashboard/NewUsers')} className="flex items-center gap-3 px-4 py-2 rounded-lg bg-blue-50 text-blue-700 font-medium">
              <UserPlus className="w-5 h-5" /> New Users
            </button>
            <button onClick={() => router.push('/dashboard/transactions')} className="flex items-center gap-3 px-4 py-2 rounded-lg hover:bg-blue-50 text-gray-700 font-medium">
              <Repeat className="w-5 h-5" /> Transactions
            </button>
            <button onClick={() => router.push('/dashboard/wallet')} className="flex items-center gap-3 px-4 py-2 rounded-lg hover:bg-blue-50 text-gray-700 font-medium">
